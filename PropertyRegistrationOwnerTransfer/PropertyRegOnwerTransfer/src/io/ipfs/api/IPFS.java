...
public class IPFS {
	...
	public List<String> ObjectPatchTypes = Arrays.asList("add-link", "rm-link",
		...
		try {
			...
			if (detected.isBefore(MIN_VERSION))
				throw new IllegalStateException(
						"You need to use a more recent version of IPFS! >= "
								+ MIN_VERSION);
		} catch (IOException e) {
	...
	public List<MerkleNode> add(NamedStreamable file, boolean wrap)
			throws IOException {
		return add(file, wrap, false);
	}

	public List<MerkleNode> add(NamedStreamable file, boolean wrap,
			boolean hashOnly) throws IOException {
		return add(Collections.singletonList(file), wrap, hashOnly);
	}

	public List<MerkleNode> add(List<NamedStreamable> files, boolean wrap,
			boolean hashOnly) throws IOException {
		Multipart m = new Multipart(protocol + "://" + host + ":" + port
				+ version + "add?stream-channels=true&w=" + wrap + "&n="
				+ hashOnly, "UTF-8");
	...
	public List<MerkleNode> ls(Multihash hash) throws IOException {
		...
		return ((List<Object>) reply.get("Objects"))
				.stream()
				.flatMap(
						x -> ((List<Object>) ((Map) x).get("Links")).stream()
	...
	public List<Multihash> refs(Multihash hash, boolean recursive)
			throws IOException {
		String jsonStream = new String(retrieve("refs?arg=" + hash + "&r="
				+ recursive));
		return JSONParser.parseStream(jsonStream).stream()
				.map(m -> (String) (((Map) m).get("Ref"))).map(Cid::decode)
				.collect(Collectors.toList());
	}

	public Map resolve(String scheme, Multihash hash, boolean recursive)
			throws IOException {
		return retrieveMap("resolve?arg=/" + scheme + "/" + hash + "&r="
				+ recursive);
	}
	...
	public Map mount(java.io.File ipfsRoot, java.io.File ipnsRoot)
			throws IOException {
		if (ipfsRoot != null && !ipfsRoot.exists())
			ipfsRoot.mkdirs();
		if (ipnsRoot != null && !ipnsRoot.exists())
			ipnsRoot.mkdirs();
		return (Map) retrieveAndParse("mount?arg="
				+ (ipfsRoot != null ? ipfsRoot.getPath() : "/ipfs") + "&arg="
	...
	public class Refs {
		public List<Multihash> local() throws IOException {
			...
			Stream<Object> s = JSONParser.parseStream(jsonStream).stream();
			List<Multihash> list = new ArrayList<>();
			for (Iterator iterator = s.iterator(); iterator.hasNext();) {
				LinkedHashMap multihash = (LinkedHashMap) iterator.next();
				try {
					list.add(Cid.decode(multihash.get("Ref").toString()));
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	public class Pin {
		public List<Multihash> add(Multihash hash) throws IOException {
			return ((List<Object>) ((Map) retrieveAndParse("pin/add?stream-channels=true&arg="
					+ hash)).get("Pins")).stream()
		...
		public Map<Multihash, Object> ls(PinType type) throws IOException {
			return ((Map<String, Object>) (((Map) retrieveAndParse("pin/ls?stream-channels=true&t="
					+ type.name())).get("Keys")))
					.entrySet()
					.stream()
					.collect(
							Collectors.toMap(x -> Cid.decode(x.getKey()),
									x -> x.getValue()));
		}
		...
		public List<Multihash> rm(Multihash hash, boolean recursive)
				throws IOException {
			Map json = retrieveMap("pin/rm?stream-channels=true&r=" + recursive
					+ "&arg=" + hash);
			return ((List<Object>) json.get("Pins")).stream()
					.map(x -> Cid.decode((String) x))
					.collect(Collectors.toList());
		}

		public List<MultiAddress> update(Multihash existing,
				Multihash modified, boolean unpin) throws IOException {
			return ((List<Object>) ((Map) retrieveAndParse("pin/update?stream-channels=true&arg="
					+ existing + "&arg=" + modified + "&unpin=" + unpin))
					.get("Pins")).stream()
	 ...
	 */
	public class Key {
		public KeyInfo gen(String name, Optional<String> type,
				Optional<String> size) throws IOException {
			return KeyInfo.fromJson(retrieveAndParse("key/gen?arg=" + name
					+ type.map(t -> "&type=" + t).orElse("")
					+ size.map(s -> "&size=" + s).orElse("")));
		}

		public List<KeyInfo> list() throws IOException {
			return ((List<Object>) ((Map) retrieveAndParse("key/list"))
					.get("Keys")).stream().map(KeyInfo::fromJson)
					.collect(Collectors.toList());
		}

		public Object rename(String name, String newName) throws IOException {
			return retrieveAndParse("key/rename?arg=" + name + "&arg="
					+ newName);
		}

		public List<KeyInfo> rm(String name) throws IOException {
			return ((List<Object>) ((Map) retrieveAndParse("key/rm?arg=" + name))
					.get("Keys")).stream().map(KeyInfo::fromJson)
					.collect(Collectors.toList());
		}
	}
	...
	public class Pubsub {
		 ...
		 * @param topic
		 * @param data
		 *            url encoded data to be published
		 * @return
		 * @throws IOException
		 */
		...
		public Stream<Map<String, Object>> sub(String topic,
				ForkJoinPool threadSupplier) throws Exception {
			return retrieveAndParseStream("pubsub/sub?arg=" + topic,
					threadSupplier).map(obj -> (Map) obj);
		}
		 ...
		 * @throws IOException
		 */
		public void sub(String topic, Consumer<Map<String, Object>> results,
				Consumer<IOException> error) throws IOException {
			retrieveAndParseStream("pubsub/sub?arg=" + topic,
					res -> results.accept((Map) res), error);
		}
	...
	public class Block {
		...
		public List<MerkleNode> put(List<byte[]> data, Optional<String> format)
				throws IOException {
			// N.B. Once IPFS implements a bulk put this can become a single
			// multipart call with multiple 'files'
		...
		public MerkleNode put(byte[] data, Optional<String> format)
			...
			Multipart m = new Multipart(protocol + "://" + host + ":" + port
					+ version + "block/put?stream-channels=true" + fmt, "UTF-8");
			try {
				m.addFilePart("file", Paths.get(""),
						new NamedStreamable.ByteArrayWrapper(data));
				String res = m.finish();
				return JSONParser.parseStream(res).stream()
						.map(x -> MerkleNode.fromJSON((Map<String, Object>) x))
						.findFirst().get();
	 ...
	 * directly. {Object} is a subset of {Block}
	 */
	public class IPFSObject {
		public List<MerkleNode> put(List<byte[]> data) throws IOException {
			Multipart m = new Multipart(protocol + "://" + host + ":" + port
					+ version + "object/put?stream-channels=true", "UTF-8");
			for (byte[] f : data)
				m.addFilePart("file", Paths.get(""),
						new NamedStreamable.ByteArrayWrapper(f));
			String res = m.finish();
			return JSONParser.parseStream(res).stream()
					.map(x -> MerkleNode.fromJSON((Map<String, Object>) x))
					.collect(Collectors.toList());
		...
		public List<MerkleNode> put(String encoding, List<byte[]> data)
			...
			if (!"json".equals(encoding) && !"protobuf".equals(encoding))
				throw new IllegalArgumentException(
						"Encoding must be json or protobuf");
			Multipart m = new Multipart(protocol + "://" + host + ":" + port
					+ version + "object/put?stream-channels=true&encoding="
					+ encoding, "UTF-8");
			for (byte[] f : data)
				m.addFilePart("file", Paths.get(""),
						new NamedStreamable.ByteArrayWrapper(f));
		...
		public MerkleNode get(Multihash hash) throws IOException {
			Map json = retrieveMap("object/get?stream-channels=true&arg="
					+ hash);
		...
		public MerkleNode links(Multihash hash) throws IOException {
			Map json = retrieveMap("object/links?stream-channels=true&arg="
					+ hash);
			return MerkleNode.fromJSON(json);
		}
		...
		public MerkleNode _new(Optional<String> template) throws IOException {
			if (template.isPresent()
					&& !ObjectTemplates.contains(template.get()))
				throw new IllegalStateException("Unrecognised template: "
						+ template.get());
			Map json = retrieveMap("object/new?stream-channels=true"
					+ (template.isPresent() ? "&arg=" + template.get() : ""));
			return MerkleNode.fromJSON(json);
		}

		public MerkleNode patch(Multihash base, String command,
				Optional<byte[]> data, Optional<String> name,
				Optional<Multihash> target) throws IOException {
			if (!ObjectPatchTypes.contains(command))
				throw new IllegalStateException(
						"Illegal Object.patch command type: " + command);
			String targetPath = "object/patch/" + command + "?arg="
					+ base.toBase58();
			if (name.isPresent())
			...
			case "add-link":
				if (!target.isPresent())
					throw new IllegalStateException(
							"add-link requires name and target!");
			case "rm-link":
				if (!name.isPresent())
			...
			case "append-data":
				...
				Multipart m = new Multipart(protocol + "://" + host + ":"
						+ port + version + "object/patch/" + command + "?arg="
						+ base.toBase58() + "&stream-channels=true", "UTF-8");
				m.addFilePart("file", Paths.get(""),
						new NamedStreamable.ByteArrayWrapper(data.get()));
				String res = m.finish();
		...
		public MerkleNode patch2(Multihash base, String command,
				Optional<byte[]> data, Optional<String> name,
				Optional<Multihash> target) throws IOException {
			if (!ObjectPatchTypes.contains(command))
				throw new IllegalStateException(
	...
	public class Name {
		...
		public Map publish(Multihash hash, Optional<String> id)
				throws IOException {
			return retrieveMap("name/publish?arg=/ipfs/" + hash
					+ id.map(name -> "&key=" + name).orElse(""));
		}
	...
	public class DHT {
		public List<Map<String, Object>> findprovs(Multihash hash)
				throws IOException {
	...
	public class Bootstrap {
		...
		public List<MultiAddress> add(MultiAddress addr) throws IOException {
			return ((List<String>) retrieveMap("bootstrap/add?arg=" + addr)
					.get("Peers")).stream().map(x -> new MultiAddress(x))
					.collect(Collectors.toList());
		}
		...
		public List<MultiAddress> rm(MultiAddress addr, boolean all)
				throws IOException {
			return ((List<String>) retrieveMap(
					"bootstrap/rm?" + (all ? "all=true&" : "") + "arg=" + addr)
					.get("Peers")).stream().map(x -> new MultiAddress(x))
					.collect(Collectors.toList());
		}
	}
			...
			return ((Map<String, Object>) m.get("Addrs"))
					...
					.collect(
							Collectors.toMap(
									...
									e -> ((List<String>) e.getValue()).stream()
	...
	public class Dag {
		...
		public MerkleNode put(String inputFormat, byte[] object)
				throws IOException {
			return put(inputFormat, object, "cbor");
		}

		public MerkleNode put(byte[] object, String outputFormat)
				throws IOException {
		...
		public MerkleNode put(String inputFormat, byte[] object,
			...
			Multipart m = new Multipart(prefix
					+ "dag/put/?stream-channels=true&input-enc=" + inputFormat
					+ "&f=" + outputFormat, "UTF-8");
			m.addFilePart("file", Paths.get(""),
					new NamedStreamable.ByteArrayWrapper(object));
			String res = m.finish();
			return MerkleNode.fromJSON(JSONParser.parse(res));
		}
	}
	...
	public class Config {
		...
		public void replace(NamedStreamable file) throws IOException {
			Multipart m = new Multipart(protocol + "://" + host + ":" + port
					+ version + "config/replace?stream-channels=true", "UTF-8");
			m.addFilePart("file", Paths.get(""), file);
			String res = m.finish();
		}
	...
	private Stream<Object> retrieveAndParseStream(String path,
			ForkJoinPool executor) throws IOException {
	 ...
	 * @throws IOException
	 */
	private void retrieveAndParseStream(String path, Consumer<Object> results,
			Consumer<IOException> err) throws IOException {
		getObjectStream(retrieveStream(path),
				d -> results.accept(JSONParser.parse(new String(d))), err);
	}
		...
		try {
			...
			throw new RuntimeException(
					"IOException contacting IPFS daemon.\nTrailer: "
							+ conn.getHeaderFields().get("Trailer") + " " + err,
					e);
		}
	}

	private void getObjectStream(InputStream in, Consumer<byte[]> processor,
	...
	private Map postMap(String path, byte[] body, Map<String, String> headers)
			throws IOException {
		URL target = new URL(protocol, host, port, version + path);
		return (Map) JSONParser.parse(new String(post(target, body, headers)));
	}

	private static byte[] post(URL target, byte[] body,
